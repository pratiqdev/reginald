...
const streamFileAndTest = (_file_data, _tests, PKG, BY_CALL_LIST, settings) => {
    return new Promise((res, rej) => {
        const stream = fs.createReadStream(_file_data.path, { highWaterMark: 10485760 });
        ...
        let mapTestToMatches = {};
        let wordRefs = [];
        let mapWordRef = {};
        let mapWordRefUnique = {};
        // let match_array:any[] = []
        let matchRefs = [];
        let mapLineBreaksToIndex = {};
        let mapFailedFileToTest = {};
        // let CONDITION:any[] = []
        ...
        //     )
        // }
        const runTest = async (test, data = false) => {
            ...
            if (BY_CALL_LIST.includes(TEST_NAME) && !data)
                return;
            ...
            while ((match = REGEX.exec(tempChunk)) !== null) {
                ...
                let actual_line = parseInt(Object.entries(mapLineBreaksToIndex).find(([lb, i]) => i <= actual_index)[0] || '1');
            ...
            if (testResults[TEST_NAME].condition && 'call' in TEST_DATA) {
                await Promise.all(TEST_DATA.call.map((testName) => {
            ...
            const runSpellChecker = () => {
                ...
                try {
                    ...
                    SC_results.forEach((res) => {
                        ...
                        let line_find = Object.entries(mapLineBreaksToIndex).find(([lb, i]) => i <= actual_index) ?? ['1'];
        ...
        stream.on('close', async () => {
            ...
            if (!settings.middleWareBeforeTests) {
                await Promise.all(Object.entries(settings.middleware).map(async ([MW_NAME, MW_FUNC]) => {
                    testResults[MW_NAME] = await MW_FUNC({
                        file: _file_data,
                        tests: _tests
                    });
