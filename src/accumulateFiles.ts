...
const accumulateFiles = async (config: IAccumulateFilesConfig = { paths: ['.'] }) => {
    ...
    const getFiles = async (dir:any) => {
        ...
        const files:any[] = await Promise.all(dirents.map(async (dirent) => {
            if(
